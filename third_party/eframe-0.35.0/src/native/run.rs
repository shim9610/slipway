...
use super::winit_integration::SlipwayDebugUserEvent;
...
impl<T: WinitApp> ApplicationHandler<UserEvent> for WinitAppWrapper<T> {
    ...
    fn user_event(&mut self, event_loop: &ActiveEventLoop, event: UserEvent) {
        profiling::function_scope!(match &event {
            ...
            #[cfg(feature = "slipway_debug")]
            UserEvent::SlipwayDebugInput { .. } => "UserEvent::SlipwayDebugInput",
            #[cfg(feature = "slipway_debug")]
            UserEvent::SlipwayDebugCapture { .. } => "UserEvent::SlipwayDebugCapture",
            #[cfg(feature = "slipway_debug")]
            UserEvent::SlipwayDebugWake { .. } => "UserEvent::SlipwayDebugWake",
        });
        ...
        event_loop_context::with_event_loop_context(event_loop, move || {
            let event_result = match event {
                ...
                #[cfg(feature = "slipway_debug")]
                UserEvent::SlipwayDebugInput { viewport_id, plan } => {
                    self.winit_app.on_slipway_debug_user_event(
                        event_loop,
                        SlipwayDebugUserEvent::Input { viewport_id, plan },
                    )
                }
                #[cfg(feature = "slipway_debug")]
                UserEvent::SlipwayDebugCapture {
                    ...
                    event_loop,
                    SlipwayDebugUserEvent::Capture {
                        viewport_id,
                        request,
                    },
                ),
                #[cfg(feature = "slipway_debug")]
                UserEvent::SlipwayDebugWake { viewport_id, token } => {
                    self.winit_app.on_slipway_debug_user_event(
                        event_loop,
                        SlipwayDebugUserEvent::Wake { viewport_id, token },
                    )
                }
            };
            self.handle_event_result(event_loop, event_result);
...
#[cfg(all(test, feature = "slipway_debug", target_os = "windows"))]
mod slipway_debug_event_pump_tests {
    ...
    use winit::{
        platform::{
            run_on_demand::EventLoopExtRunOnDemand as _, windows::EventLoopBuilderExtWindows as _,
        },
        window::{Window, WindowAttributes},
    };

    use super::*;
    use crate::native::winit_integration::SlipwayDebugUserEvent;

    #[derive(Default)]
    struct NoopWake;

    impl egui_wgpu::winit::DirectCaptureWake for NoopWake {
    ...
    struct PumpApp {
        ...
        capture: Option<egui_wgpu::winit::DirectCaptureRequest>,
        seen_input: bool,
        seen_capture: bool,
    ...
    impl PumpApp {
        ...
        fn repaint_next(&self) -> EventResult {
            self.window.as_ref().map_or(EventResult::Wait, |window| {
                EventResult::RepaintNext(window.id())
            })
        }
    }

    impl WinitApp for PumpApp {
        fn egui_ctx(&self) -> Option<&egui::Context> {
            self.egui_winit.as_ref().map(egui_winit::State::egui_ctx)
        }
        ...
        fn window_id_from_viewport_id(&self, id: egui::ViewportId) -> Option<WindowId> {
            (id == egui::ViewportId::ROOT)
                .then(|| self.window.as_ref().map(|window| window.id()))
                .flatten()
        }
        ...
        fn suspended(&mut self, _event_loop: &ActiveEventLoop) -> crate::Result<EventResult> {
            Ok(EventResult::Wait)
        }
        ...
        fn on_accesskit_event(
            &mut self,
            _event: egui_winit::accesskit_winit::Event,
        ) -> crate::Result<EventResult> {
            Ok(EventResult::Wait)
        }
        ...
        fn on_slipway_debug_user_event(
            ...
            match event {
                SlipwayDebugUserEvent::Input { viewport_id, plan } => {
                    ...
                    self.egui_winit
                        .as_mut()
                        .unwrap()
                        .ingest_slipway_debug_input(self.window.as_ref().unwrap(), plan)
                        .unwrap();
            ...
            Ok(self.repaint_next())
        }
    }

    fn queue_fixture_events(
        proxy: &crate::NativeDebugProxy,
    ) -> mpsc::Receiver<egui_wgpu::winit::DirectCaptureEvent> {
        proxy
            .try_send_input(
                egui::ViewportId::ROOT,
                egui_winit::SlipwayDebugInputPlan {
                    token: 71,
                    events: vec![egui_winit::SlipwayDebugInputEvent::Copy],
                },
            )
            .unwrap();
        ...
        proxy
            ...
            .unwrap();
        proxy.wake(egui::ViewportId::ROOT, 73).unwrap();

        event_rx
    }

    #[test]
    fn application_handler_forwards_input_capture_and_wake_before_take() {
        let mut builder = EventLoop::<UserEvent>::with_user_event();
        builder.with_any_thread(true);
        let mut event_loop = builder.build().unwrap();
        let debug_proxy = crate::NativeDebugProxy::new(&event_loop);
        let _event_rx = queue_fixture_events(&debug_proxy);

        let mut wrapper = WinitAppWrapper::new(PumpApp::new(), true);
        event_loop.run_app_on_demand(&mut wrapper).unwrap();

        assert!(wrapper.winit_app.seen_input);
        assert!(wrapper.winit_app.seen_capture);
        assert_eq!(wrapper.winit_app.wake_token, Some(73));
        assert_eq!(wrapper.winit_app.capture.as_ref().unwrap().token, 72);

        let (raw_input, notice) = wrapper
            .winit_app
            .egui_winit
            .as_mut()
            .unwrap()
            .take_egui_input_with_slipway_debug(wrapper.winit_app.window.as_ref().unwrap());
        assert!(matches!(
            notice,
            Some(egui_winit::SlipwayDebugInputNotice::Accepted(
                egui_winit::SlipwayDebugInputSpan {
                    token: 71,
                    start_event_index: 0,
                    end_event_index: 1,
                }
        ...
        assert!(matches!(raw_input.events.as_slice(), [egui::Event::Copy]));
    }
}

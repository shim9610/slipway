...
pub enum UserEvent {
    ...
    /// Request-scoped input to ingest before taking a viewport's egui input.
    #[cfg(feature = "slipway_debug")]
    SlipwayDebugInput {
        /// Target viewport.
        viewport_id: ViewportId,
        /// Input plan to normalize through egui-winit.
        plan: egui_winit::SlipwayDebugInputPlan,
    },

    /// Direct capture of the next visible presentation for a viewport.
    #[cfg(feature = "slipway_debug")]
    SlipwayDebugCapture {
        /// Target viewport.
        viewport_id: ViewportId,
        /// Request consumed by the viewport's wgpu painter.
        request: egui_wgpu::winit::DirectCaptureRequest,
    },

    /// A capture result or deadline wake for a viewport.
    #[cfg(feature = "slipway_debug")]
    SlipwayDebugWake {
        /// Target viewport.
        viewport_id: ViewportId,
        /// Request token carried for app-owned correlation.
        token: u64,
    },
}
...
/// Internal ownership-preserving payload forwarded to the selected native renderer.
#[cfg(feature = "slipway_debug")]
#[derive(Debug)]
pub enum SlipwayDebugUserEvent {
    /// Input to ingest into viewport-owned egui-winit state.
    Input {
        /// Target viewport.
        viewport_id: ViewportId,
        /// Input plan.
    ...
    /// Capture request to arm on the target viewport.
    Capture {
        /// Target viewport.
        viewport_id: ViewportId,
    ...
    /// Capture result or deadline wake.
    Wake {
        /// Target viewport.
        viewport_id: ViewportId,
        /// Request token.

...
/// Identifies a WGPU frame after its surface texture was presented.
#[cfg(feature = "slipway_debug")]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SlipwayDebugPostPresent {
    /// The viewport whose surface texture was presented.
    pub viewport_id: egui::ViewportId,

    /// The direct-capture token, or `None` for an ordinary presentation.
    pub capture_token: Option<u64>,
}

/// Sends Slipway debug input and capture requests through eframe's native event loop.
#[cfg(all(not(target_arch = "wasm32"), feature = "slipway_debug"))]
#[derive(Clone)]
pub struct NativeDebugProxy {
    event_loop_proxy: winit::event_loop::EventLoopProxy<UserEvent>,
}

#[cfg(all(not(target_arch = "wasm32"), feature = "slipway_debug"))]
impl NativeDebugProxy {
    /// Creates a proxy for the given eframe event loop.
    pub fn new(event_loop: &winit::event_loop::EventLoop<UserEvent>) -> Self {
        Self {
            event_loop_proxy: event_loop.create_proxy(),
        }
    }

    /// Queues request-scoped input for a viewport.
    ///
    /// # Errors
    ///
    /// Returns the unsent event when the native event loop is closed.
    pub fn try_send_input(
        &self,
        viewport_id: egui::ViewportId,
        plan: egui_winit::SlipwayDebugInputPlan,
    ) -> Result<(), winit::event_loop::EventLoopClosed<UserEvent>> {
        self.event_loop_proxy
            .send_event(UserEvent::SlipwayDebugInput { viewport_id, plan })
    }

    /// Arms direct capture of the next visible presentation for a viewport.
    ///
    /// # Errors
    ///
    /// Returns the unsent event when the native event loop is closed.
    pub fn try_send_capture(
        ...
        self.event_loop_proxy
            .send_event(UserEvent::SlipwayDebugCapture {
                viewport_id,
                request,
            })
    }

    /// Wakes a viewport after a direct-capture event or deadline.
    ///
    /// # Errors
    ///
    /// Returns the unsent event when the native event loop is closed.
    pub fn wake(
        ...
        self.event_loop_proxy
            .send_event(UserEvent::SlipwayDebugWake { viewport_id, token })
    }
}
...
pub trait App {
    ...
    /// Called with the dependency-produced Slipway input notice paired to this raw input.
    #[cfg(feature = "slipway_debug")]
    fn raw_input_hook_with_slipway_debug(
        &mut self,
        ctx: &egui::Context,
        raw_input: &mut egui::RawInput,
        _notice: Option<&egui_winit::SlipwayDebugInputNotice>,
    ) {
        self.raw_input_hook(ctx, raw_input);
    }

    /// Called synchronously after WGPU presents a viewport surface texture.
    #[cfg(feature = "slipway_debug")]
    fn on_slipway_debug_post_present(&mut self, _event: SlipwayDebugPostPresent) {}
}

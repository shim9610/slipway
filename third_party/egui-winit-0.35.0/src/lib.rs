...
#[derive(Clone, Debug)]
pub struct SlipwayDebugInputPlan {
    pub token: u64,
    pub events: Vec<SlipwayDebugInputEvent>,
}

#[cfg(feature = "slipway_debug")]
#[derive(Clone, Debug)]
pub enum SlipwayDebugInputEvent {
    CursorMoved {
...
#[cfg(feature = "slipway_debug")]
#[derive(Clone, Debug)]
pub struct SlipwayDebugKeyEvent {
...
pub struct SlipwayDebugInputSpan {
    pub token: u64,
...
#[cfg(feature = "slipway_debug")]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum SlipwayDebugInputError {
    Busy,
    Unsupported,
    InvalidImeRange,
}
...
#[cfg(feature = "slipway_debug")]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum SlipwayDebugInputNotice {
    Accepted(SlipwayDebugInputSpan),
    Refused {
        token: u64,
        error: SlipwayDebugInputError,
    },
}

#[cfg(feature = "slipway_debug")]
fn validate_slipway_debug_input_events(
    events: &[SlipwayDebugInputEvent],
) -> Result<(), SlipwayDebugInputError> {
    for event in events {
        match event {
            SlipwayDebugInputEvent::MouseButton { button, .. }
                if translate_mouse_button(*button).is_none() =>
            {
                return Err(SlipwayDebugInputError::Unsupported);
            }
            SlipwayDebugInputEvent::Keyboard(event) => {
                let physical_key = match event.physical_key {
                    winit::keyboard::PhysicalKey::Code(keycode) => key_from_key_code(keycode),
                ...
                let has_printable_text = text
                    .is_some_and(|text| !text.is_empty() && text.chars().all(is_printable_char));
                if logical_key.or(physical_key).is_none() && !has_printable_text {
                    return Err(SlipwayDebugInputError::Unsupported);
                }
            }
            SlipwayDebugInputEvent::Ime(winit::event::Ime::Preedit(
                text,
                Some((start_bytes, end_bytes)),
            )) if text.get(..*start_bytes).is_none()
                || text.get(*start_bytes..*end_bytes).is_none() =>
            {
                return Err(SlipwayDebugInputError::InvalidImeRange);
            }
            _ => {}
        }
    }
...
pub struct State {
    ...
    #[cfg(feature = "slipway_debug")]
    slipway_debug_input_notice: Option<SlipwayDebugInputNotice>,
...
impl State {
    ...
    pub fn new(
        ...
        let mut slf = Self {
            ...
            egui_input,
            #[cfg(feature = "slipway_debug")]
            slipway_debug_input_notice: None,
    ...
    #[cfg(feature = "slipway_debug")]
    /// Append one request-scoped debug plan to the current native input accumulator.
    ///
    /// # Errors
    ///
    /// Returns [`SlipwayDebugInputError::Busy`] while a prior notice is pending,
    /// [`SlipwayDebugInputError::Unsupported`] for input egui cannot represent, or
    /// [`SlipwayDebugInputError::InvalidImeRange`] for a non-UTF-8-boundary preedit range.
    pub fn ingest_slipway_debug_input(
        &mut self,
        window: &winit::window::Window,
        plan: SlipwayDebugInputPlan,
    ) -> Result<SlipwayDebugInputSpan, SlipwayDebugInputError> {
        let pixels_per_point = pixels_per_point(&self.egui_ctx, window);
        self.ingest_slipway_debug_input_with_pixels_per_point(pixels_per_point, plan)
    }

    #[cfg(feature = "slipway_debug")]
    fn ingest_slipway_debug_input_with_pixels_per_point(
        &mut self,
        pixels_per_point: f32,
        plan: SlipwayDebugInputPlan,
    ) -> Result<SlipwayDebugInputSpan, SlipwayDebugInputError> {
        if self.slipway_debug_input_notice.is_some() {
            return Err(SlipwayDebugInputError::Busy);
        }

        let token = plan.token;
        if let Err(error) = validate_slipway_debug_input_events(&plan.events) {
            self.slipway_debug_input_notice =
                Some(SlipwayDebugInputNotice::Refused { token, error });
            return Err(error);
        }

        let start_event_index = self.egui_input.events.len();
        for event in plan.events {
            match event {
                SlipwayDebugInputEvent::CursorMoved { position } => {
                    self.on_cursor_moved_with_pixels_per_point(pixels_per_point, position);
                }
                SlipwayDebugInputEvent::MouseButton { state, button } => {
                    self.on_mouse_button_input(state, button);
                }
                SlipwayDebugInputEvent::MouseWheel { delta, phase } => {
                    self.on_mouse_wheel_with_pixels_per_point(pixels_per_point, delta, phase);
                }
                SlipwayDebugInputEvent::Modifiers(state) => self.on_modifiers_changed(state),
                SlipwayDebugInputEvent::Keyboard(event) => {
                    self.on_keyboard_input_fields(
                        event.physical_key,
                        &event.logical_key,
                        event.text.as_deref(),
                        event.location,
                ...
                SlipwayDebugInputEvent::Ime(ime) => self.on_ime(&ime),
                SlipwayDebugInputEvent::Copy => self.egui_input.events.push(egui::Event::Copy),
                SlipwayDebugInputEvent::Cut => self.egui_input.events.push(egui::Event::Cut),
                SlipwayDebugInputEvent::PointerGone => self.on_pointer_gone(),
            }
        }

        let span = SlipwayDebugInputSpan {
            token,
            start_event_index,
            end_event_index: self.egui_input.events.len(),
        };
        self.slipway_debug_input_notice = Some(SlipwayDebugInputNotice::Accepted(span));
        Ok(span)
    }

    #[cfg(feature = "slipway_debug")]
    pub fn take_egui_input_with_slipway_debug(
        &mut self,
        window: &winit::window::Window,
    ) -> (egui::RawInput, Option<SlipwayDebugInputNotice>) {
        let notice = self.slipway_debug_input_notice.take();
        (self.take_egui_input(window), notice)
    }
    ...
    pub fn on_window_event(
        ...
        match event {
            ...
            WindowEvent::ModifiersChanged(state) => {
                self.on_modifiers_changed(state.state());
    ...
    fn on_modifiers_changed(&mut self, state: winit::keyboard::ModifiersState) {
    ...
    fn on_cursor_moved(
        ...
        let pixels_per_point = pixels_per_point(&self.egui_ctx, window);
        self.on_cursor_moved_with_pixels_per_point(pixels_per_point, pos_in_pixels);
    }

    fn on_cursor_moved_with_pixels_per_point(
        &mut self,
        pixels_per_point: f32,
        pos_in_pixels: winit::dpi::PhysicalPosition<f64>,
    ) {
    ...
    fn on_mouse_wheel_with_pixels_per_point(
        &mut self,
        pixels_per_point: f32,
        delta: winit::event::MouseScrollDelta,
        phase: winit::event::TouchPhase,
    ...
    fn on_keyboard_input_fields(
        &mut self,
        winit_physical_key: winit::keyboard::PhysicalKey,
        winit_logical_key: &winit::keyboard::Key,
        text: Option<&str>,
        _location: winit::keyboard::KeyLocation,
        state: winit::event::ElementState,
        _repeat: bool,
    ) {
        let pressed = state == winit::event::ElementState::Pressed;

        let physical_key = if let winit::keyboard::PhysicalKey::Code(keycode) = winit_physical_key {
        ...
        log::trace!(
            "logical {winit_logical_key:?} -> {logical_key:?},  physical \
             {winit_physical_key:?} -> {physical_key:?}"
        );
        ...
        if let Some(text) = text.or_else(|| winit_logical_key.to_text()) {
...
mod slipway_debug_tests {
    ...
    struct UnavailableDisplay;

    impl raw_window_handle::HasDisplayHandle for UnavailableDisplay {
        fn display_handle(
            &self,
    ...
    fn key_event(character: &str, code: winit::keyboard::KeyCode) -> SlipwayDebugKeyEvent {
        SlipwayDebugKeyEvent {
            physical_key: winit::keyboard::PhysicalKey::Code(code),
            logical_key: winit::keyboard::Key::Character(character.into()),
    ...
    #[test]
    fn invalid_ime_range_is_atomic_and_keeps_one_notice() {
        let mut state = state_without_window();
        state.egui_input.events.push(egui::Event::Copy);
        let original_events = state.egui_input.events.clone();

        let result = state.ingest_slipway_debug_input_with_pixels_per_point(
            1.0,
            SlipwayDebugInputPlan {
                token: 7,
                events: vec![
        ...
        assert_eq!(result, Err(SlipwayDebugInputError::InvalidImeRange));
        assert_eq!(state.egui_input.events, original_events);
        assert_eq!(
            state.slipway_debug_input_notice,
            Some(SlipwayDebugInputNotice::Refused {
                token: 7,
                error: SlipwayDebugInputError::InvalidImeRange,
            })
        );

        let busy = state.ingest_slipway_debug_input_with_pixels_per_point(
            1.0,
            SlipwayDebugInputPlan {
                token: 8,
                events: vec![SlipwayDebugInputEvent::Copy],
            },
        );
        assert_eq!(busy, Err(SlipwayDebugInputError::Busy));
        assert_eq!(
            state.slipway_debug_input_notice,
            Some(SlipwayDebugInputNotice::Refused {
                token: 7,
    ...
    #[test]
    fn unsupported_input_is_refused_before_mutation() {
        let mut state = state_without_window();
        let result = state.ingest_slipway_debug_input_with_pixels_per_point(
            1.0,
            SlipwayDebugInputPlan {
                ...
                events: vec![
                    ...
                    SlipwayDebugInputEvent::MouseButton {
                        state: winit::event::ElementState::Pressed,
        ...
        assert_eq!(result, Err(SlipwayDebugInputError::Unsupported));
        assert!(state.egui_input.events.is_empty());
    }

    #[test]
    fn ime_lifecycle_without_raw_events_preserves_empty_span() {
        let mut state = state_without_window();
        let span = state
            .ingest_slipway_debug_input_with_pixels_per_point(
                1.0,
                SlipwayDebugInputPlan {
                    token: 10,
                    events: vec![
                        SlipwayDebugInputEvent::Ime(winit::event::Ime::Enabled),
                        SlipwayDebugInputEvent::Ime(winit::event::Ime::Disabled),
                    ],
                },
            )
            .expect("IME lifecycle plan should be accepted");

        assert_eq!(
            span,
            SlipwayDebugInputSpan {
                token: 10,
                start_event_index: 0,
                end_event_index: 0,
            }
        ...
        assert_eq!(
            state.slipway_debug_input_notice,
            Some(SlipwayDebugInputNotice::Accepted(span))
        );
        assert!(state.egui_input.events.is_empty());
    }

    #[test]
    fn unicode_ime_range_uses_native_converter() {
        let mut state = state_without_window();
        let span = state
            .ingest_slipway_debug_input_with_pixels_per_point(
                1.0,
                SlipwayDebugInputPlan {
                    token: 13,
                    events: vec![SlipwayDebugInputEvent::Ime(winit::event::Ime::Preedit(
                        "aé日".to_owned(),
                        Some((1, 3)),
                    ))],
                },
            )
            .expect("valid Unicode IME range should be accepted");

        assert_eq!(span.start_event_index, 0);
        assert_eq!(span.end_event_index, 1);
        assert!(matches!(
            &state.egui_input.events[0],
            egui::Event::Ime(egui::ImeEvent::Preedit {
                text,
                active_range_chars: Some(range),
            }) if text == "aé日" && range == &(1..2)
        ));
    }

    #[test]
    fn native_and_debug_keyboard_use_the_same_fields_normalizer() {
        let event = key_event("a", winit::keyboard::KeyCode::KeyA);
        let mut native_state = state_without_window();
        native_state.on_keyboard_input_fields(
            event.physical_key,
            &event.logical_key,
            event.text.as_deref(),
            event.location,
        ...
        let mut debug_state = state_without_window();
        let span = debug_state
            .ingest_slipway_debug_input_with_pixels_per_point(
                1.0,
                SlipwayDebugInputPlan {
                    token: 11,
                    events: vec![SlipwayDebugInputEvent::Keyboard(event)],
                },
            )
            .expect("representable debug key should be accepted");

        assert_eq!(
            native_state.egui_input.events,
            debug_state.egui_input.events
        );
        assert_eq!(span.start_event_index, 0);
        assert_eq!(span.end_event_index, 2);
    }

    #[test]
    fn idle_native_normalization_does_not_create_debug_state() {
        let mut state = state_without_window();
        let event = key_event("z", winit::keyboard::KeyCode::KeyZ);
        state.on_keyboard_input_fields(
            event.physical_key,
            &event.logical_key,
            event.text.as_deref(),
            event.location,
        ...
        assert_eq!(state.slipway_debug_input_notice, None);
    }

    #[cfg(target_os = "windows")]
    #[test]
    fn mixed_native_debug_events_keep_span_through_take() {
        ...
        let event_loop = event_loop_builder
            .build()
            .expect("test event loop should be available");
        #[expect(deprecated)]
        ...
        let native_before = key_event("a", winit::keyboard::KeyCode::KeyA);
        state.on_keyboard_input_fields(
            native_before.physical_key,
            &native_before.logical_key,
            native_before.text.as_deref(),
            native_before.location,
            native_before.state,
            native_before.repeat,
        );
        let span = state
            .ingest_slipway_debug_input(
                &window,
                SlipwayDebugInputPlan {
                    token: 12,
                    events: vec![SlipwayDebugInputEvent::Keyboard(key_event(
                        "a",
                        winit::keyboard::KeyCode::KeyA,
                    ))],
                },
            )
            .expect("mixed debug key should be accepted");
        let native_after = key_event("a", winit::keyboard::KeyCode::KeyA);
        state.on_keyboard_input_fields(
            native_after.physical_key,
            &native_after.logical_key,
            native_after.text.as_deref(),
            native_after.location,
            native_after.state,
            native_after.repeat,
        );

        let (raw_input, notice) = state.take_egui_input_with_slipway_debug(&window);
        assert_eq!(
            span,
            SlipwayDebugInputSpan {
                token: 12,
        ...
        assert_eq!(notice, Some(SlipwayDebugInputNotice::Accepted(span)));
        assert_eq!(raw_input.events.len(), 6);
        assert_eq!(state.slipway_debug_input_notice, None);
    }
}
